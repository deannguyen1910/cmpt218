...
// void *first_fit2(int size, void* ptr){
//     if ()
//     struct header *tempNode = ptr;
//     if (tempNode->next != NULL){
//         return first_fit2(size, tempNode->next);
//     }
// }
...
void *initializeSpace(struct header *mem, int size){
    ...
    if (mem->next != NULL){
        ...
        if (heapinfo.free_size - size > HEADER_SIZE){ // case 
            struct header *tempNode = (char*)mem + HEADER_SIZE + size;
            mem->size = size + HEADER_SIZE;
            mem->next = tempNode;
            heapinfo.free_size -= mem->size;
            return (char *)mem + HEADER_SIZE;
        }else{ // case at the end of the link list
            mem->size = heapinfo.free_size + HEADER_SIZE;
            mem->next = NULL;
            heapinfo.free_size -= mem->size - HEADER_SIZE;
            return (char*)mem + HEADER_SIZE;
        }
    }
}
...
void dealloc(void *ptr){
    if (ptr - HEADER_SIZE != memhead){
        ...
        if (heapinfo.free_size % INCREMENT == 0){
            struct header* tempNode = startHeap;
            tempNode->next = NULL;
            tempNode->size = 0;
            memhead = startHeap;
            heapinfo.free_size -= HEADER_SIZE;
